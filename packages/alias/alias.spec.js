...
import { nextResolve } from '../../fixtures/nextResolve.fixture.js';
...
describe('alias', () => {
	...
	describe('that are in tsconfig.json', async () => {
		...
		before(async () => {
			...
			({ resolve } = await import('./alias.loader.js'));
		});

		await test('should de-alias a prefixed specifier', () => {
			assert.equal(
				resolve('…/test.js', {}, nextResolve).url,
				`${base}/src/test.js`,
			);
		});
		...
		await test('should maintain any suffixes on the prefixed specifier', () => {
			assert.equal(
				resolve('…/test.js?foo', {}, nextResolve).url,
				`${base}/src/test.js?foo`,
			);
			assert.equal(
				resolve('…/test.js#bar', {}, nextResolve).url,
				`${base}/src/test.js#bar`,
			);
			assert.equal(
				resolve('…/test.js?foo#bar', {}, nextResolve).url,
				`${base}/src/test.js?foo#bar`,
			);
		});
