...
import { assertSuffixedSpecifiers } from '../../fixtures/assert-suffixed-specifiers.fixture.js';
...
const tsxExts = new Set(['.ts', '.ts', '.tsx']);
...
describe('JSX & TypeScript loader', { concurrency: true, skip }, () => {
	...
	before(async () => {
		...
		({ load, resolve } = await import('./tsx.loader.js'));
	});
